...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    _LOGGER.info(f"[{DOMAIN}] Inicializando integración v1.1.8 - Filtro de Notificaciones de Apagado")
    ...
    async def evaluar_logica(_=None):
        try:
            ...
            # CONTROL DE NOTIFICACIÓN DE INICIO
            if is_on and not enchufe_estaba_on:
                ...
                if is_red or is_solar:
                    msg = "de Red (100%)." if is_red else "mediante producción solar (80%)."
            ...
            if is_on:
                ...
                # Corte del 80% blindado contra duplicados
                ...
                # Corte del 100% blindado con reseteo inmediato de ticks
                tiempo_encendido = ahora - data.get("timestamp_encendido", ahora)
                if (is_red or is_solar) and tiempo_encendido > 60.0:
                    if 0 < val_potencia < 15:
                        data["bms_ticks"] += 1
                        if data["bms_ticks"] >= 3:
                            data["bms_ticks"] = 0  # <--- ¡Paso 1: Ponemos a cero el contador inmediatamente!
                            await guardar_estado() # <--- ¡Paso 2: Lo consolidamos en la memoria interna!
                            
                            await hass.services.async_call("homeassistant", "turn_off", {"entity_id": conf_enchufe})
                            await hass.services.async_call("homeassistant", "turn_off", {"entity_id": sw_red})
                            await hass.services.async_call("homeassistant", "turn_off", {"entity_id": sw_solar})
                            await enviar_msg("🔋 100% alcanzado", "corriente cortada.")
                            return
                    else:
                        data["bms_ticks"] = 0
                else:
            ...
            _LOGGER.error(f"[{DOMAIN}] Error en evaluar_logica: {e}")
    ...
    entidades_a_vigilar = [sw_solar, sw_red, conf_enchufe, conf_solar, num_umbral]
    entry.async_on_unload(async_track_state_change_event(hass, entidades_a_vigilar, state_listener))

    async def timer_callback(now):
        await evaluar_logica()
    
    entry.async_on_unload(async_track_time_interval(hass, timer_callback, timedelta(seconds=10)))
